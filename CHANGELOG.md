...
### Added
- docs/user/DATA_FLOWS.md: what Lyra captures, keeps, and the optional network outputs with their switches.
- VOICES.md and docs/licenses/: Piper voice licences, Llama 3.2 Community License and use policy ("Built with Llama").
- THIRD_PARTY_LICENSES.md generated by `make licenses` and checked in CI.
- README: English summary, prerequisites table, security guarantees mapped to code and tests, clickable metrics, ecosystem block.
- ROADMAP.md.

### Changed
- Packaging: hatchling, distribution name `lyra-assistant`, dependencies in pyproject with `gpu`/`dev` extras and a uv lockfile; CUDA libraries are no longer imposed.
- CI: reusable workflow (unit + installer tests on 3.11/3.12, ruff, personal-path guard), Dependabot.
- systemd units shipped with `@LYRA_DIR@` / `@HOME@` placeholders; no personal path left in code, scripts or tests.
- Tracking client defaults come from `TRACKING_SERVER_SCRIPT` / `TRACKING_VENV_PYTHON`; RAG spec sources from `LYRA_MCP_SPECS_DIR`.
