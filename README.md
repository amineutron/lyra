...
|---|---|---|
| Système | Fedora 42, Ubuntu 24.04 ou Arch (testés en VM, [protocole](docs/user/VM_INSTALL_TESTS.md)) | idem |
| Python | 3.11 | 3.12 |
| RAM | 8 Go | 16 Go |
...
| Commandes hors règles, petit modèle | **21/21** sur le banc des modèles et **50/51** sur le jeu de formulations inédites (5 serveurs) avec `qwen2.5-coder:0.5b`, contre 5/21 et 13/51 avant la [boucle d'amélioration](docs/dev/BOUCLE_AMELIORATION.md) ([BENCHMARKS.md](BENCHMARKS.md), `make bench-modeles`). Sur un **cinquième jeu de 50 formulations tenu à l'écart** (jamais itéré, vrais noms de machines), la configuration finale fait **35/50** ; c'est la mesure de généralisation à retenir, les quatre autres jeux ayant servi à la boucle (le quatrième, mesuré une fois à 41/50, a guidé la dernière itération ; le troisième, 100 formulations, est passé de 56 à 83/100 en trois itérations) |
...
| Local par défaut : aucune sortie réseau sans configuration explicite | [`docs/user/DATA_FLOWS.md`](docs/user/DATA_FLOWS.md) | vérification par `grep` décrite dans le document |
